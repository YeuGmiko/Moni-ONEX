...
import type {RuleViewItem} from '@/components/common/RuleView/types'
import {reactive, ref} from 'vue'
import {OhVueIcon} from 'oh-vue-icons'
import {useMessage} from 'naive-ui'
import {useRouter} from 'vue-router'
import RuleView from '@/components/common/RuleView/RuleView.vue'
...
const nicknameRules: CustomRules = [
  { label: '长度为2-16之间', rule: new RegExp(/^.{2,16}$/), error: '长度必须为2-16之间' }
]
  ...
  name: [
    {
      required: true,
      trigger: ['input', 'blur'],
      validator: getRuleValidator(nicknameRules)
    }
  ],
  userName: [
    {
...
async function submit() {
  ...
  const { error, response } = await fetchRegisterUser(model)
  ...
  <NForm ref="formRef" :show-label="false" :rules="rules" v-model:model="model">
    <NFormItem path="name">
      <NPopover placement="top-start" trigger="focus" display-directive="show">
        <template #trigger>
          <NInput placeholder="请输入用户昵称" v-model:value="model.name">
            <template #prefix><OhVueIcon name="si-personio" scale="1.1" fill="gray"></OhVueIcon></template>
          </NInput>
        </template>
        <template #default>
          <RuleView :rules="nicknameRules" :value="model.name"></RuleView>
        </template>
      </NPopover>
    </NFormItem>
