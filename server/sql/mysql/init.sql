...
INSERT INTO map_schema(id, map_key, map_value, remark)
VALUES  (1926461261290143746, 'system_gen_question_count', 50, '每日生成的目标题目数量');

CREATE TABLE daily_task(
                           id bigint UNSIGNED COMMENT '任务ID',
                           task_date datetime NOT NULL DEFAULT NOW() COMMENT '任务日期',
                           create_time datetime NOT NULL DEFAULT NOW() COMMENT '创建时间',
                           PRIMARY KEY (id),
                           question_id bigint UNSIGNED COMMENT '题目ID'
) ENGINE=innodb CHARACTER SET utf8mb4 COLLATE utf8mb4_bin COMMENT '每日任务列表';

DROP TABLE IF EXISTS `daily_task_option_submit`;
CREATE TABLE daily_task_option_submit(
                                         id bigint UNSIGNED COMMENT '任务提交ID',
                                         submit_date datetime NOT NULL DEFAULT NOW() COMMENT '提交日期',
                                         user_answer varchar(255) NOT NULL COMMENT '用户提交的答案',
                                         is_right bool NOT NULL DEFAULT false COMMENT '是否正确：便于数据处理',
                                         order_no int3 UNSIGNED NOT NULL COMMENT '选项顺序',
                                         PRIMARY KEY (id),
                                         option_id bigint UNSIGNED NOT NULL COMMENT '选项ID',
                                         question_id bigint UNSIGNED NOT NULL COMMENT '题目ID：便于查询',
                                         user_id bigint UNSIGNED NOT NULL COMMENT '用户ID',
                                         task_id bigint UNSIGNED NOT NULL COMMENT '任务ID'
) ENGINE=innodb CHARACTER SET utf8mb4 COLLATE utf8mb4_bin COMMENT '每日任务提交列表';