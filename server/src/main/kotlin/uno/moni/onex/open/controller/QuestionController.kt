...
class QuestionController(
    ...
    fun getQuestions(@PathVariable moduleId: String): Response<List<OpenQuestionVo>> {
        val userId: String? = try {
            StpUtil.getLoginId().toString()
        } catch(e: Exception) {
            null
        }
        return Response.success().data(questionService.loadOpenedByModuleId(moduleId, userId))
    }
