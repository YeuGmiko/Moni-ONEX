...
import { useQuestionStore, useModuleStore } from '@/store'
...
const { getTarget: getModule } = moduleStore
const { getData: getQuestionData, getTarget: getQuestion, toNext: toNextQuestion, submit: submitQuestion, init: initQuestion } = questionStore
const { current: currentModule } = storeToRefs(moduleStore)
const { data: questionData, isLoading: isQuestionLoading, current: currentQuestion, nextTarget: nextQuestion, isSubmit, answers } = storeToRefs(questionStore)
...
const markdown = computed<string>(() => currentQuestion.value?.content ?? '')
...
async function init() {
  ...
  await getQuestionData(moduleId)
  await getQuestionByQuery()
}

async function getQuestionByQuery() {
  if (isQuestionLoading.value.data || isQuestionLoading.value.target) return
...
async function submit() {
  ...
  if (!result) return
  await submitQuestion(Array.from(Object.values(model.value)))
}
...
async function handleOrderChoose(order: OrderProperties) {
  ...
  await getQuestion(order.id)
}

watch(currentQuestion, async question => {
  if (!question || !question.id || !currentModule.value || !currentModule.value.id) return
  await router.push({
    name: ROUTE_NAME.modules,
    params: { id: currentModule.value.id },
...
/* 页面更换 / 刷新的时候，触发题目信息获取*/
watch(route, getQuestionByQuery)
/* 检测是否已经提交，若提交则重新渲染表单 */
watch(isSubmit, isSubmit => {
  if (!isSubmit || !currentQuestion.value) return
  const { userSubmits } = currentQuestion.value
...
<template>
  <div class="m-5 flex-grow flex gap-x-5 mx-auto max-w-[1640px] w-90%">
    <div class="flex-grow flex flex-col">
      ...
      <div v-if="isQuestionLoading.target || isQuestionLoading.data"
            ...
            <NScrollbar v-if="isSubmit" x-scrollable :size="2" content-class="py-1.5">
              ...
              <span v-if="wrongAnswers[getSubmitOptionKey(option.orderNo)]"
                ...
                <span :class="['px-1', 'text-white', 'rounded-0.5', 'cursor-pointer']"
                  :style="{ backgroundColor: currentModule?.bgColor }"
              ...
              <span v-else-if="isSubmit" class="ml-2 is-right-answer" :style="{
                '--theme-color': currentModule?.bgColor
              }">正确</span>
            </NScrollbar>
          </NFormItem>
          <NFormItem>
            <div class="flex gap-x-2">
              <NButton v-if="!isSubmit" type="primary" :color="currentModule?.bgColor" @click="submit">提交</NButton>
              <NButton v-if="nextQuestion" class="items-center" type="primary" :color="currentModule?.bgColor" text
    ...
    <div v-if="currentModule && currentQuestion">
      <ModuleTag :theme-color="currentModule?.bgColor" :label="currentModule?.label"></ModuleTag>
      <QuestionOrders :theme-color="currentModule.bgColor"
                      :tag-label="currentModule.label" :current="currentQuestion" :orders="questionData" @choose="handleOrderChoose">
      </QuestionOrders>
    </div>
  </div>
</template>
